...
if cors_origins_env:
    ...
    # Get the host IP from environment or use common localhost/network IPs
    ...
    # If API_HOST is set to a network IP, add common frontend ports for that IP
    if api_host != "127.0.0.1" and api_host != "localhost":
        for port in [3000, 5173, 1420, 8000]:
            cors_origins.append(f"http://{api_host}:{port}")
...
@app.post("/upload")
async def upload_document(request: Request, current_user: str = Depends(get_current_user)):
    """Upload a document (placeholder - minimal implementation)"""
    # The frontend expects this endpoint but full upload/processing is out of scope
    # This is a minimal implementation that returns a placeholder response
    logger.info("Upload endpoint called (minimal implementation)")
    
    # Try to get file from form data if present
    try:
        ...
        if file:
            ...
            # In a full implementation, this would process the file
            # For now, return a placeholder response
            return {
                "status": "accepted",
                "message": f"File {filename} received (processing not implemented)",
                "document_id": f"doc-{uuid.uuid4().hex[:8]}"
            }
    except Exception as e:
        logger.warning(f"Error handling upload: {str(e)}")
    
    raise HTTPException(
        status_code=501,
        detail={
            "error": "Upload functionality not fully implemented",
            "code": "NOT_IMPLEMENTED",
            "details": {"message": "Use process_upload_folder.py or raganything_example.py to process documents"}
        }
    )

@app.get("/{filename}/status")
async def get_upload_status(filename: str, current_user: str = Depends(get_current_user)):
    """Get processing status for an uploaded file (placeholder)"""
    logger.info(f"Status check requested for: {filename} by user: {current_user}")
